...
class DescribeListingVC: UIViewController, UITableViewDelegate, UITableViewDataSource, GetListingTypeDelegate, UIGestureRecognizerDelegate {
    ...
    @IBAction func click_NextButton(_ sender: Any) {
        ...
        status.count != 0 ? (self.screnComingFrom == "spaceLocated" ? self.pushToSpaceDimensionController() : self.pushToFeatureController())  : alert(message: NSLocalizedString("Select atleast one item", comment: ""), Controller: self)
    }
